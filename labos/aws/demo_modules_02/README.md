...
main.tf
  │
  ├── module "dev"      → modules/environment
  │                           ├── module "networking" → modules/networking
  │                           └── module "compute"    → modules/compute
  │
  ├── module "staging"  → modules/environment (même module !)
  │                           ├── module "networking"
  │                           └── module "compute"
  │
  └── module "prod"     → modules/environment (même module !)
                              ├── module "networking"
                              └── module "compute"


Avantage clé → pour ajouter un environnement qa demain, il suffit d'ajouter 5 lignes dans le main.tf